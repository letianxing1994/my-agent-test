...
wss.on("connection", (ws: WebSocket) => {
	...
	ws.on("message", (data: Buffer) => {
		try {
			...
			console.log(`收到消息: ${message.type} 来自: ${message.senderId}`);

			// 处理Agent注册
			if (message.type === MessageType.STATUS_UPDATE && message.content) {
				const content = message.content as any;
				if (content.action === "register") {
					activeAgents.set(message.senderId, ws);
					console.log(`✅ Agent已注册: ${message.senderId} (${content.name})`);
					console.log(`📊 当前活跃Agent数: ${activeAgents.size}`);
					...
					const ackMessage: AgentMessage = {
						...
						content: { action: "registered", status: "success" } as unknown as JsonValue,
	...
	ws.on("close", () => {
		// 移除断开连接的Agent
		for (const [agentId, agentWs] of activeAgents.entries()) {
			if (agentWs === ws) {
				activeAgents.delete(agentId);
				console.log(`❌ Agent已断开: ${agentId}`);
				console.log(`📊 当前活跃Agent数: ${activeAgents.size}`);
				break;
			}
		}
